...
\\\\Simulations can often be time consuming and require hours or days to complete in order to achieve acceptable accuracy. It is common to construct super-computers in order to achieve better performance, and so less time to run a simulation. However, this is often not economical or widely available. One way to improve performance without simply adding more computational power is to use Graphics Processing Units (GPUs). Unlike CPUs, GPUs run computations in parallel which can make simulations a lot faster if the same calculations are run many times.
...
\subsection{Drag}
This is the drag verification case from Section \ref{sec:drag verification}. The properties used are in Figure \ref{fig:python drag properties} and the results are shown in Figure \ref{fig:python_drag_verification}. The results show that the simulation results closely match the analytic solution and that the error decreases as the timestep decreases.
\begin{figure}[!htb]
\centering
\begin{tabular}{| c c |}
\hline
...
\label{fig:python drag properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
\label{fig:python normal collision properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
\label{fig:python friction sliding properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
\subsection{Drag}
This is the drag verification case from Section \ref{sec:drag verification}. The properties used are in Figure \ref{fig:opencl drag properties} and the results are shown in Figure \ref{fig:opencl_drag_verification}. The results show that the simulation results closely match the analytic solution and that the error decreases as the timestep decreases.
\begin{figure}[!htb]
\centering
\begin{tabular}{| c c |}
\hline
...
\label{fig:opencl drag properties}
\end{figure}
\begin{figure}[!htb]
\centering
\includegraphics[scale=0.5]{figures/opencl_verification/drag_verification.png}
...
\end{figure}
\subsection{Normal Collision}
This is the normal collision verification case from Section \ref{sec:normal collision verification}. The properties used are in Figure \ref{fig:opencl normal collision properties} and the results are shown in \ref{fig:opencl_normal_force_verification}. The results show that the simulation results closely match the analytic solution and that the error decreases as the timestep decreases.
\begin{figure}[!htb]
\centering
\begin{tabular}{| c c |}
\hline
...
\label{fig:opencl normal collision properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
\label{fig:opencl friction sliding properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
\label{fig:opencl normal collision with cohesion properties}
\end{figure}
\begin{figure}[!htb]
\centering
...
This section contains the constant properties used in the simulations. Note that, due to the long simulation durations, the timestep is only $t_{collision} / 8$. Although this is not of the highest accuracy, it does allow for longer simulations and for more data points to be collected. Absolute accuracy is not as important since the focus of these simulations is to see how agglomerates vary with different properties. A smaller timestep would be recommended if these simulations were required to predict specific outcomes of real scenarios.
\begin{figure}[!htb]
\centering
\begin{tabular}{| c c |}
\hline
